# Terraform tu zip infra/lambda/ — khong phai zip tay roi nho commit.
# Dieu kien: da chay `npm run build` de co node_modules dung kien truc.
...
# sharp co binary theo kien truc, nen phai cai dung linux/arm64 (Graviton) —
# cai tren macOS roi zip len se loi "Could not load the sharp module" luc chay,
# khong phai luc deploy. precondition o duoi bat loi do ngay tu terraform plan.
#
...
resource "aws_lambda_function" "thumbnailer" {
  ...
  # Handler Node.js la `<duong-dan-khong-extension>.<ten-export>`: index.mjs
  # export `handler`, nhung Runtime setting phai la src/index.handler.
...
# Tao truoc bang Terraform de dat retention. De Lambda tu tao thi log giu mai
# mai va hoa don CloudWatch cu the tang.
